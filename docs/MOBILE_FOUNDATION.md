...
The next slices add an end-to-end development journey in that shared shell: a member can enter an approved server address, sign in, choose one of their available clubs, view the dog profiles they are permitted to see, open a profile with its audience label and photograph, edit profiles they own or have delegated care permission for, and replace or remove their photograph. The client receives a minimal allowlisted projection and never receives owner account IDs, care notes, password hashes or stored session hashes.
