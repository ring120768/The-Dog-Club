...
The shared shell now provides an end-to-end development journey: a member can enter an approved server address, sign in, choose one of their available clubs, view the dog profiles they are permitted to see, open a profile with its audience label and photograph, edit profiles they own or have delegated care permission for, replace or remove their photograph, review membership benefits and upcoming grooming bookings, search live grooming availability and confirm an appointment. The client receives a minimal allowlisted projection and never receives owner account IDs, care notes, password hashes, stored session hashes, staff identities or resource identifiers.

Booking uses the existing server-side eligibility, staffing, qualification, shift, break, closure, resource and concurrency rules. The primary account may apply an eligible grooming credit when enough usable credits remain. A household adult with booking permission can book the shared dog but cannot spend the primary account's credits. The member must accept the service's snapshotted cancellation terms. If no credit is applied, the app shows the GBP amount due at the club and states that no payment is taken in this demo. A member can cancel their future booking after an explicit confirmation; the server audits the cancellation, releases the slot and restores an applied grooming credit once.
...
The current shared shell covers member sign-in, club selection, dog profiles and photographs, authorised profile/photo editing, current membership and upcoming bookings, live grooming availability, booking confirmation and cancellation. It keeps the bearer session in memory and uses the operating-system file picker without camera or broad media permissions.

Exercise the complete journey on one iOS simulator and one Android emulator against a reviewed HTTPS non-production deployment. Next, design atomic rescheduling and decide whether service payment should remain pay-at-club or move to a reviewed Stripe flow. Decide whether an in-app camera is justified. Add camera permissions only with reviewed privacy text and denial/retry behaviour. Decide shared marketplace listing versus operator-branded releases before creating signing identities or promising individual store listings.
